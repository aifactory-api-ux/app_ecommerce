...
export async function getTopProducts(
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(startDate)) throw new Error('start_date must be an ISO date string')
  if (!/^\d{4}-\d{2}-\d{2}$/.test(endDate)) throw new Error('end_date must be an ISO date string')
